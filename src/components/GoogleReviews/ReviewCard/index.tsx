...
import { getGoogleReviewPageUrl } from "../constants";

const reviewLinkClass =
  "swiper-no-swiping cursor-pointer transition-opacity hover:opacity-80";
...
const ReviewCard = ({ review }: ReviewCardProps) => {
  const reviewUrl = getGoogleReviewPageUrl(review);
    ...
    <article className="flex h-full min-h-[250px] flex-col rounded-[8px] bg-[#F5F5F5] p-5">
      <div className="flex items-center gap-3">
        <a
          ...
          className={`shrink-0 ${reviewLinkClass}`}
          aria-label={`View ${review.authorName}'s review on Google`}
        >
        ...
        <div className="min-w-0">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className={`block truncate text-[14px] font-semibold text-[#302A25] hover:underline ${reviewLinkClass}`}
          >
            {review.authorName}
          </a>
      ...
      {review.text && (
        <div className="mt-3">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className={`mt-1 inline-block text-[12px] font-medium text-[#4285F4] hover:underline ${reviewLinkClass}`}
          >
            Read more
          </a>
        </div>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className={`mt-auto flex w-fit items-center gap-2 pt-4 text-[11px] text-[#858585] hover:underline ${reviewLinkClass}`}
        aria-label={`View ${review.authorName}'s review on Google`}
      >
