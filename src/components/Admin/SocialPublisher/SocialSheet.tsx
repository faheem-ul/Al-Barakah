...
type SocialSheetProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onLogout: () => void;
};

const SocialSheet: React.FC<SocialSheetProps> = ({
  open,
  onOpenChange,
  onLogout,
}) => (
  <Sheet open={open} onOpenChange={onOpenChange}>
    ...
    >
      <SheetHeader className="px-4 py-4 border-b border-black/10 text-left shrink-0">
        <SheetTitle className="sr-only">Social Publisher</SheetTitle>
        <SheetDescription className="sr-only">
          Admin social publisher navigation
        </SheetDescription>
